...
$isGt3160 = version_compare(Settings::get('nomad_version_installed'), '1.2.0', '>=');
// Not installed
if (!Settings::get('nomad_version_installed')) {
    new G\Handler([
...
if (is_readable(G_APP_PATH . 'nomadImages-hook.php')) {
    require_once G_APP_PATH . 'nomadImages-hook.php';
}
...
try {
    if (!isset($hook_before)) {
        $hook_before = function ($handler) {
            ...
            $pageHandle = version_compare(Settings::get('nomad_version_installed'), '1.2.0', '>=') ? 'internal' : 'url_key';
