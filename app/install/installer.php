...
try {
    if (!is_null(getSetting('nomad_version_installed')) and !Login::getUser()['is_admin']) {
    ...
    $settings_updates = [
        ...
        '1.0.0' => [
            'analytics_code' => null,
            'auto_language' => 1,
            'nomad_version_installed' => G_APP_VERSION,
            'cloudflare' => null,
            'comment_code' => null,
            ...
            'website_description' => 'A free image hosting service powered by NomadImages',
            'website_doctitle' => 'NomadImages image hosting',
            'website_name' => 'NomadImages',
            'website_explore_page' => 1,
            'twitter_account' => 'NomadImages',
            'enable_signups' => 1,
            ...
            'website_keywords' => 'image sharing, image hosting, NomadImages',
...
UPDATE `%table_prefix%users` SET user_content_views = COALESCE((SELECT SUM(image_views) FROM `%table_prefix%images` WHERE image_user_id = user_id GROUP BY user_id), "0");';
    ...
    $installed_version = getSetting('nomad_version_installed');
    ...
    if ($installed_version && !$_POST) {
        ...
        if ((!empty($db_settings_keys) && count($chv_initial_settings) !== count($db_settings_keys)) || (!is_null($installed_version) and version_compare(G_APP_VERSION, $installed_version, '>'))) {
            if (!array_key_exists(G_APP_VERSION, $settings_updates)) {
                die('Fatal error: app/install is outdated. You need to re-upload app/install folder with the one from NomadImages ' . G_APP_VERSION);
            }
            // Get database schema
            $schema = [];
...
WHERE NOT EXISTS (SELECT * FROM `%table_prefix%imports` WHERE `import_path`='%rootPath%importing/parse-albums' AND `import_continuous`=1 LIMIT 1);",
            ...
            $sql_update[] = 'UPDATE `%table_prefix%settings` SET `setting_value` = "' . G_APP_VERSION . '" WHERE `setting_name` = "nomad_version_installed";';
            ...
            try {
                ...
                $updated = $db->exec();
                if ($updated) {
                    $nomad_version_installed = DB::get('settings', ['name' => 'nomad_version_installed'])[0]['setting_value'];
                    if (G_APP_VERSION !== $nomad_version_installed) {
    ...
    $doctitle = $doctitles[$doing] . ' - NomadImages ' . get_chevereto_version(true);
