...
<p>At this point you already have all the needed files uploaded and now you need to indicate where NomadImages will store the data (MySQL database).</p>
...
<div data-modal="modal-db-info" class="hidden">
	...
	<p>NomadImages will connect to your database host using the database user credentials so this user must have access to the target database name.</p>
</div>
<?php if($error) { ?>
...
<form method="post" class="c9">
	...
	<div class="input-label">
		...
		<input type="text" name="db_name" id="db_name" class="text-input" value="<?php echo $safe_post['db_name'] ?? ''; ?>" placeholder="Database name" title="Name of the database where you want to install NomadImages" rel="tooltip" data-tipTip="right" required>
	</div>
	<div class="input-label">
